...
git init
git add .
git commit -m "feat: BLU打印中转 App 初始版本"
git remote add origin https://github.com/YOUR_USERNAME/*-bluprint.git
git branch -M main
...
# App 位于：
# build/macos/Build/Products/Release/*_print.app
```
...
```bash
# 拖到 Applications 文件夹
cp -R build/macos/Build/Products/Release/*_print.app /Applications/
...
Web 端（企业微信管理系统）通过 HTTP 与本 App 通信：
...
async function printLabel(base64Data, copies = 1) {
  try {
    ...
    // 提示用户检查打印中转 App 是否运行
    alert('无法连接打印服务，请确保「BLU 打印中转」App 已启动');
  }
}
...
async function safePrint(base64Data, copies) {
  ...
  if (!status.connected) {
    alert('打印机未连接，请打开「BLU打印中转」App 连接打印机');
    return;
  }
